...
var annotation = function (type, args) {
   ...
   *    coordinates.
   * @param {boolean|'open'} [addEdges] If truthy, add a halfway point between
   *    each of vertex.  If `addEdges === 'open'`, don't add an edge between
   *    the first and last vertex.
   * @param {boolean} [addCenter] If truthy, add a center based on the
   *    specified annotation's coordinates.
   * @param {boolean} [addRotate] If truthy, add a rotation handle outside of
   *    the area populated by the annotation's coordinates.  Note that this is
   *    positioned based on the current display coordinates of the annotation,
   *    so subsequent calls to this function could result in different results,
   *    even if the annotation has not changed.
   * @param {boolean} [addResize] If truthy, add a resize handle outside of
   *    the area populated by the annotation's coordinates.  This is positioned
   *    similarly to the rotation handle.
   */
  this._addEditHandles = function (features, vertices, addEdges, addCenter, addRotate, addResize) {
    var editPoints,
    ...
    switch (state) {
      ...
      default:
        ...
        if (state === annotationState.edit) {
          m_this._addEditHandles(features, opt.corners, true, true, true, true);
        }
        break;
    }
    return features;
  ...
  this.features = function () {
    ...
    switch (state) {
      ...
      default:
        ...
        if (state === annotationState.edit) {
          m_this._addEditHandles(features, opt.vertices, true, true, true, true);
        }
        break;
    }
    return features;
  ...
  this.features = function () {
    ...
    switch (state) {
      ...
      default:
        ...
        if (state === annotationState.edit) {
          m_this._addEditHandles(features, opt.vertices, m_this.style('closed') ? true : 'open', true, true, true);
        }
        break;
    }
    return features;
